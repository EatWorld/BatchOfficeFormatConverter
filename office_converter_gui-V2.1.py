...
3. Select additional options as needed:
   ...
   • Overwrite Original Files Directly: Delete original files after conversion, keeping only new format files
...
© 2024 Yanshan Township People's Government, Bengshan District, Bengbu City"""
    ...
    def on_overwrite_change(self, *args):
        """处理覆盖原文件选项变化"""
    ...
    def create_widgets(self):
        ...
        # 创建主内容区域（调整高度：800-50标题-150底部进度=600像素）
        main_frame = tk.Frame(self.root, bg=COLORS['background'], height=600)
    ...
    def create_left_panel(self):
        ...
        # 第一行：备份和覆盖选项
        archive_row1 = tk.Frame(archive_frame, bg=COLORS['surface'])
        archive_row1.pack(fill="x", pady=2)
        
        archive_cb = self.create_modern_checkbox(archive_row1, "备份到默认文件夹", self.archive_originals)
        archive_cb.pack(side="left", padx=(0, 20))
        
        overwrite_cb = self.create_modern_checkbox(archive_row1, "转换后直接覆盖原文件", self.overwrite_original)
        overwrite_cb.pack(side="left")
        
        # 第二行：自定义备份选项
        archive_row2 = tk.Frame(archive_frame, bg=COLORS['surface'])
        archive_row2.pack(fill="x", pady=2)
        
        custom_archive_cb = self.create_modern_checkbox(archive_row2, "备份到自定义文件夹", self.use_custom_archive)
        custom_archive_cb.pack(side="left", padx=(0, 0))
        ...
        self.overwrite_original.trace('w', self.on_overwrite_change)
    ...
    def create_left_panel_english(self):
        ...
        # First row: backup and overwrite options
        archive_row1 = tk.Frame(archive_frame, bg=COLORS['surface'])
        archive_row1.pack(fill="x", pady=2)
        
        archive_cb = self.create_modern_checkbox(archive_row1, "Backup to Default Folder", self.archive_originals)
        archive_cb.pack(side="left", padx=(0, 20))
        
        overwrite_cb = self.create_modern_checkbox(archive_row1, "Overwrite Original Files Directly", self.overwrite_original)
        overwrite_cb.pack(side="left")
        
        # Second row: custom backup option
        archive_row2 = tk.Frame(archive_frame, bg=COLORS['surface'])
        archive_row2.pack(fill="x", pady=2)
        
        custom_archive_cb = self.create_modern_checkbox(archive_row2, "Backup to Custom Folder", self.use_custom_archive)
        custom_archive_cb.pack(side="left", padx=(0, 0))
            ...
            if self.archive_originals.get():
                ...
                if not os.path.exists(old_files_path):
                    try:
                        ...
                        old_files_path = None
            elif self.overwrite_original.get():
                self.log_message("⚠️ 注意：将直接覆盖原文件，不进行备份")
                                ...
                                if self.overwrite_original.get():
                                    ...
                                    if os.path.exists(doc_file_path):
                                        try:
                                            ...
                                            self.log_message(f"已覆盖: {file}")
                                        except Exception as e_remove:
                                            self.log_message(f"删除原文件失败: {doc_file_path} - {e_remove}")
                                elif (self.archive_originals.get() or self.use_custom_archive.get()) and old_files_path:
                                    # 备份原文件
                                    if os.path.exists(doc_file_path):
                                        try:
                                ...
                                if self.overwrite_original.get():
                                    ...
                                    if os.path.exists(xls_file_path):
                                        try:
                                            ...
                                            self.log_message(f"删除原文件失败: {xls_file_path} - {e_remove}")
                                elif (self.archive_originals.get() or self.use_custom_archive.get()) and old_files_path:
                                    # 备份原文件
                                    if os.path.exists(xls_file_path):
                                        try:
